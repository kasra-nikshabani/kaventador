...
import { LESSON_TYPE_LABELS, type Course, type LessonType } from "@/types";
...
export interface CurriculumEditorProps {
  course: Course;
  /**
   * ریشه مسیر دوره‌ها در پنلی که این ویرایشگر داخلش نشسته —
   * `/admin/courses` یا `/instructor/courses`. فقط لینک‌ها را عوض
   * می‌کند؛ اکشن‌ها یکی‌اند و مالکیت را خودشان بررسی می‌کنند.
   */
  basePath: string;
}

/**
 * ویرایشگر سرفصل: فصل‌ها، درس‌ها، ترتیب و ویدیو.
 *
 * پنل ادمین و پنل مدرس هر دو از همین کامپوننت استفاده می‌کنند. تفاوتشان
 * فقط در «چه دوره‌ای» است — و آن را صفحه تعیین می‌کند، نه این کامپوننت.
 */
export function CurriculumEditor({ course, basePath }: CurriculumEditorProps) {
  return (
                    ...
                    <ol className="divide-border divide-y">
                      {chapter.lessons.map((lesson, lessonIndex) => (
                        ...
                        >
                          ...
                          {lesson.isFree && <Badge variant="outline">رایگان</Badge>}
